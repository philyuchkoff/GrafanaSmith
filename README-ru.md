...
- `rate(node_cpu_seconds_total[5m])` копипастится везде, даже когда сервис не упирается в CPU;
- имена инстансов захардкожены вместо объявления шаблонных переменных;
- пороги выбраны случайно, без привязки к SLO.

GrafanaSmith превращает SRE best practices (RED, USE, Golden Signals) в повторяемый workflow, чтобы каждый выкатываемый дашборд имел одинаковый формат и одинаковую планку качества.
...
Каждый шаблон задаёт секции панелей, **готовые PromQL-выражения**, пороги алертов и шаблонные переменные, осмысленные для данного сервиса. Новый сервис добавляется одним файлом `templates/<service>.md` по образцу — менять сам скилл не нужно.
...
сгенерированная панель следовала одному и тому же шаблону полей, а
после выдачи JSON скилл просит импортировать дашборд и сообщить об
ошибках для второй итерации ("сгенерировал - протестили - сообщили ошибки - перегенерировал").
...
Перезапустите opencode, чтобы новый скилл подхватился.
...
> «Сделай дашборд Grafana для кластера PostgreSQL 14, который состоит из 1 primary,
> 2 replica, это production, нагрузка ~5000 QPS. Вот scrape config: ...»
...
Когда добавляете новый шаблон сервиса, старайтесь соблюдать правила:

1. **Берите метрики, которые реально эмиттит экспортер.** Сверяйтесь с
   документацией экспортера, не выдумывайте имена метрик.
2. **Используйте `$__rate_interval` в окнах `rate()`** и тогда дашборд будет
   ...
   не существует.
3. **Всегда задавайте `legendFormat`** с интерполяцией хотя бы одного лейбла. Без `legendFormat` подпись будет просто значением лейбла и нечитаемой и на графике вместо `{status="500"}` появится `Status 500`.
 Это защита от потери информации: если лейбл пропал при правке, панель перестанет показывать разумную подпись, а не молча съест ошибку.
1. В идеальном мире **пороги должны быть привязаны к SLO или к сигналу из реальных
   инцидентов.** Случайные пороги это шум. Но мир, увы, не идеален :)
2. **Группируйте панели в rows** с `collapsed: false` (по умолчанию развернута, то есть, пользователь открывает дашборд и сразу видит контент, а не кучу свёрнутых стрелок, которые надо по очереди раскликивать). Смысл правила: логически группируй панели в раскрытые по умолчанию rows, а не сбрасывай всё в один плоский лист.
3. **Переменные в snake_case и регистрозависимые.** `$replica_type`, а не
   `$ReplicaType`.
4. **Задавайте `unit` на каждой панели**. 
   Без unit Grafana показывает числа как есть: `0.0047`, `1524326400`, `987654321`. Человек не понимает, это проценты, секунды или байты. С unit Grafana форматирует: `0.47%`, `1.524 GB`, `987.65 MB/s` - то есть, автоматически подбирает приставки (K, M, G) и группирует разряды. Не ленись, ставь "unit": "s" для latency, "unit": "reqps" для throughput, "unit": "percentunit" для ratios. Процент использования CPU как 182.3% вместо 1.823 — это ошибка, которую unit предотвращает.
5.  **Следуйте конвенции `uid`** `<environment>-<service>-<purpose>`
Каждый дашборд в Grafana имеет уникальный `uid`. Если дашборды хранятся в Git (provisioning) или на них ссылаются из алертов (`dashboard_url`), `uid` должен быть предсказуемым.

`<environment>-<service>-<purpose>` схема:

- `prod-postgresql-overview`: продуктовый PG, общий дашборд
- `staging-nginx-edge`: стейджинговый NGINX, edge-слой
- `prod-myapp-stack`: composite дашборд для всего приложения

Смысл: по `uid` сразу понятно, что за дашборд, без его открытия. Алерты ссылаются на него стабильно, потому что `uid` не меняется при пересоздании.
